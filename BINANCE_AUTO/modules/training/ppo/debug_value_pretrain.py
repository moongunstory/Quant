...
def generate_rewards(returns_array: np.ndarray) -> np.ndarray:
    ...
    # logger.info(f"📊 Reward 분포: ...") # 비활성화
    # logger.info(f"   평균 reward: ...") # 비활성화
...
def calculate_horizon_returns(df_entry: pd.DataFrame, df_eval: pd.DataFrame, direction: str) -> Tuple[np.ndarray, np.ndarray]:
    ...
    for i in range(len(df_entry) - LABEL_HORIZON):
        ...
        log_this_sample = (i < 100) # DEBUG_LOG_LIMIT 대신 하드코딩
        ...
        def get_no_hit_reward(ret: float) -> float:
            for threshold, reward in no_hit_reward_table:
                if ret >= threshold:
                    return reward
            return -1.0  # fallback
        ...
        if is_tp_hit:
            ...
            reward = get_no_hit_reward(calculated_return)
...
def create_debug_pretrain_data(direction: str, sample_limit: int = 20000) -> Tuple[Dataset, Dataset, Dict]:
    logger.info(f"🛠️  [{direction.upper()}] 디버그 가치 사전 학습 데이터 준비 시작 (샘플 제한: {sample_limit}개)")
    raw_data = pd.read_pickle(TRAIN_PICKLE_PATHS[direction])

    # 데이터 샘플 제한
    limited_raw_data = {}
    for tf, df in raw_data.items():
        limited_raw_data[tf] = df.head(sample_limit)

    # 1. 리팩토링된 prepare_features 호출 (ProcessedData 객체 반환)
    processed_data = prepare_features(limited_raw_data)
    
    features_dict = processed_data.features
    input_dims = processed_data.input_dims
    final_seq_index = processed_data.final_index.dropna() 
    # logger.info(f"시퀀싱 후 최종 데이터 길이: {len(final_seq_index)}") # 비활성화
    ...
    # logger.info(f"초기 NaN 제거 후 유효 데이터 길이: {len(valid_index)}") # 비활성화
    # logger.info(f"   # logger.info(f"   # logger.info(f"   - 데이터 준비 완료: Train {len(train_dataset)}개, Val {len(val_dataset)}개 (원본 {original_data_len}개 중 유효 {filtered_data_len}개)") # 비활성화") # 비활성화") # 비활성화
...
def run_debug_value_pretraining():
    ...
    # 1. 데이터 준비
    # create_debug_pretrain_data 함수를 호출하여 실제 데이터의 일부를 사용
    train_ds, val_ds, input_dims = create_debug_pretrain_data(direction="long", sample_limit=5000) # 샘플 수 조절
    ...
    # 3. 모방학습된 정책망 가중치 로드
    imitation_policy_path = PPO_IMITATION_MODEL_PATHS["long"]
    ...
    for name, param in model.value_head.named_parameters():
        ...
        # logger.info(f"   - 학습 대상 파라미터 (가치망): {name}") # 비활성화

    # 5. 가치망 학습
    criterion = nn.MSELoss()
    # 디버깅을 위해 학습률을 높여서 테스트
    debug_lr = 3e-5
    # logger.info(f"   - 디버그 학습률 적용: {debug_lr}") # 비활성화
    optimizer = optim.Adam(model.value_head.parameters(), lr=debug_lr)
    ...
    for epoch in range(epochs):
        ...
        for features, returns in train_loader:
            ...
            _, value = model(features)
                # logger.debug(f"Value shape: {value.shape}, Returns shape: {returns.shape}") # 비활성화
        ...
        with torch.no_grad():
            for features, returns in val_loader:
                ...
                all_preds.extend(value.cpu().numpy())
