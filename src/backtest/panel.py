...
FIELD_SPECS = {
    ...
    "global_ls_accounts": ("metrics", "create_time", "count_long_short_ratio", "mean"),
    "toptrader_ls_positions": ("metrics", "create_time", "sum_toptrader_long_short_ratio", "mean"),
    "toptrader_ls_accounts": ("metrics", "create_time", "count_toptrader_long_short_ratio", "mean"),
    "premium_close": ("premiumIndexKlines", "open_time", "close", "last"),
}
...
def load_panels_for(fields, rebuild=False):
    """수식이 요구하는 field 들 + 항상 필요한 close/funding_rate/quote_volume 를 로드.

    quote_volume 은 turnover_rank 중립화(mom60_turnover_neutral 등)에 필요."""
    always = ("close", "funding_rate", "quote_volume")
...
def load_panels_for_bar(fields, bar="1d", rebuild=False):
    """수식 field + 항상필요 close/funding_rate/quote_volume 를 bar 주기로 로드.
...
def load_funding_events(rebuild=False):
    """캐시가 있으면 로드, 없거나 rebuild=True 면 새로 만들고 저장.
    실패(펀딩 데이터 없음)하면 None 을 반환 → 엔진은 펀딩 0 으로 진행."""
    path = funding_events_path()
    if path.exists() and not rebuild:
        cached = _read_cache(path)
        if cached is not None:
    ...
    try:
        ...
        log.warning("funding events 빌드 실패(펀딩 0 처리): %s", e)
        return None
    SETTINGS.panel_dir.mkdir(parents=True, exist_ok=True)
    panel.to_parquet(path)
    return panel
