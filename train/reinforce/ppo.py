...
def load_best_from_top5(top5_path: str) -> Dict[str, Any]:
    with open(top5_path, "r") as f:
        top5 = json.load(f)

    def score(trial):
        return trial["sharpe"] - 2 * trial["mdd"] + 1.5 * trial["return"]

    top5 = sorted(top5, key=score, reverse=True)
    best = top5[0]
    config = best["config"]
    config["features"] = best["features"]
    return config

def train_with_top1_from_top5(top5_path: str, train_data: Dict[str, Any], val_data: Dict[str, Any], train_steps: int = 100_000):
    config = load_best_from_top5(top5_path)
    obs_dims = {tf: len(cols) for tf, cols in config["features"].items()}

    env = MultiTimeframeTradingEnv(train_data, obs_cols=config["features"])
    eval_env = MultiTimeframeTradingEnv(val_data, obs_cols=config["features"])
    ...
    print("[done] Evaluation result:", result)
    return result
